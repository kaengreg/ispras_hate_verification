...
def build_user_prompt(question, stance, theme):
    return f""" 
...
def build_messages(question, stance, theme):
    return [{'role': 'system', 'content': SYSTEM_PROMPT},
            {'role': 'user', 'content': build_user_prompt(question, stance, theme)}]
...
def main():
    ...
    for item in data:
        ...
        base_question = item.get('generated_question')
        base_answer = item.get('text')

        if base_question and base_answer:
            candidates.append({
                'sentence_id': sentence_id,
                'link': link,
                'theme': theme,
    ...
    with open(output_rich, 'w', encoding='utf-8') as frich, \
        ...
        for c in candidates:
            ...
            messages = build_messages(c['question'], c['stance'], c['theme'])
            rich_ex = {
